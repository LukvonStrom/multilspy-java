...
async def process_diagnostics(diag):
    ...
    irrelevant_strings = ["Build path specifies execution environment", "The compiler compliance specified is"]
    # print(diag)
    if len(diag.get("diagnostics", [])) > 0:
        for d in diag.get("diagnostics", []):
            # print(d)
            if any([irrelevant_string in d.get("message", "") for irrelevant_string in irrelevant_strings]):
                continue
        ...
        async with lsp.start_server() as server:
            ...
            with server.open_file(completions_filepath):
                ...
                with open(completions_filepath, 'r', encoding="utf-8") as f:
                    ...
                    tinker_line_indexes = [i for i, line in enumerate(lines) if '(' in line and ')' in line and '//' not in line]
                ...
                assert deleted_text == remove_text, "Deleted text does not match the expected text"

                # print("Deleted text:", deleted_text, remove_text)

                completions = await server.request_completions(completions_filepath, tinker_lineno, tinker_col+1, allow_incomplete=True)
...
if __name__ == "__main__":
    ...
    if args.e and args.e.lower() == "yes":
        ...
        if editpath.exists():
            ...
            print(f"Initial file had {initial_diag_count} diagnostics, now we have {edit_diag_count} diagnostics")
